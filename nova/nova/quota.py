...
class DbQuotaDriver(object):
    ...
    def _process_quotas(self, context, resources, project_id, quotas,
        ...
        for resource in resources.values():
            ...
            #limit = limit - allocated_quotas.get(resource.name,0)
    ...
    def get_settable_quotas(self, context, resources, project_id,
        ...
        settable_quotas = {}
        db_parent_project_quotas = {}
        parent_project_quotas = {}
        project_allocated_quotas = {}
        parent_project_allocated_quotas = {}
        db_project_quotas = db.quota_get_all_by_project(context, project_id)
        project_quotas = self.get_project_quotas(context, resources,
                                                 project_id, remains=True,
                                                 project_quotas=db_project_quotas)
        project_allocated_quotas=db.quota_allocated_get_all_by_project\
                                 (context, project_id)
        if parent_id:
            db_parent_project_quotas = db.quota_get_all_by_project(context, parent_id)
            parent_project_allocated_quotas=\
                                              db.quota_allocated_get_all_by_project\
                                              (context, parent_id)
            parent_project_quotas = self.get_project_quotas(context, resources,
                                    parent_id, remains=True,
                                    project_quotas=db_parent_project_quotas)
        if user_id:
            ...
            for key, value in user_quotas.items():
                ...
                maximum =maximum - project_allocated_quotas.get(key,0)
            ...
            for key, value in project_quotas.items():
                maximum = -1
                if(parent_id):
                    maximum = parent_project_quotas[key]['remains'] - \
                    	parent_project_allocated_quotas.get(key,0) + value['limit']
                minimum = \
                    max(int(self._sub_quota_values(value['limit'],
                                                   value['remains'])),
                        int(value['in_use'] + value['reserved'])) + \
                        	project_allocated_quotas.get(key,0)
                settable_quotas[key] = {'minimum': minimum, 'maximum': maximum}
